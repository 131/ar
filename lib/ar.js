...
class AnimationRecorder extends Events.EventEmitter {
  ...
  * run() {
    ...
    if(self.config.capture_mode == "swf") {
      ...
      if(self.config.swf_version == "1.0")
    ...
    var screenInfos = yield promisify(winapi.GetDisplaySettings, winapi)();
    var screen = {
    ...
    if(this.config.orientation == "auto") {
      ...
      if(!isValidSize(size, screen)) {
        ...
        if(screen.orientation == 0 || screen.orientation == 2)
          yield promisify(winapi.ReOrientDisplay, winapi)(1);
        else
          yield promisify(winapi.ReOrientDisplay, winapi)(0);
      }
    }


    var fps = self.config.swf_version == "1.0" ? 20 : 25;
  ...
  * startGUI() {
   ...
   appframe.once('loaded', function(){
      ...
      var timeout = self.config.duration * 1000;
      console.log("Will timeout after", self.config.duration);
