  ...
  * startGUI() {
   ...
   appframe.once('loaded', function(){
      ...
      if(self.config.capture_mode == "swf"  && self.config.swf_version == "1.0") {
        ...
        setTimeout(appframe.window.dispatchEvent.bind(appframe.window), self.config.duration * 1000 + 200, new Event("AR_END"));
      } else {
        setTimeout(appframe.window.dispatchEvent.bind(appframe.window), self.config.duration * 1000, new Event("AR_END"));
      }
