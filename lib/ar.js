...
class AnimationRecorder extends Events.EventEmitter {
  ...
  async run() {
    ...
    await new Promise((resolve) => {
      this._screenrecorder.warmup(resolve);
    });
  ...
  async startGUI() {
    ...
    this._screenrecorder.once(SR.EVENT_DONE, (err, path) => {
      console.log("This is path %s, err is %s into ", path, err, this.config.output_path);

      mv(path, this.config.output_path, function() {
        appframe.close();
      });
    });
    ...
    appframe.once('loaded', () => {
      ...
      appframe.window.addEventListener("AR_BEGIN", () => {
        this._screenrecorder.StartRecord(function() {
          console.log("Record started");
        });
      });


      appframe.window.addEventListener("AR_END", () => {
        this._screenrecorder.StopRecord(function() {
