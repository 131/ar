...
class AnimationRecorder extends Events.EventEmitter {
  ...
  async run() {
    ...
    var screenInfos = await promisify(screenRotation.GetDisplaySettings, screenRotation)();
    var screen = {
    ...
    if(this.config.orientation == "auto") {
      ...
      if(!isValidSize(size, screen)) {
        ...
        if(screen.orientation == 0 || screen.orientation == 2)
          await promisify(screenRotation.ReOrientDisplay, screenRotation)(1);
        else
          await promisify(screenRotation.ReOrientDisplay, screenRotation)(0);
      }
    }
